...
export const Following = () => {
    const {
        selectedUser, setSelectedUser, isFollowingRequested, setIsFollowingRequested,
        setFetchingFollowing, fetchingFollowing, followingPagesCount
    } = useContext(Context)
    ...
    useEffect(() => {
        if (fetchingFollowing && currentPage <= followingPagesCount) {
            axios
                .get<SearchUsertype[]>(`https://api.github.com/users/${selectedUser?.login}/following?page=${currentPage}&per_page=21`)
                .then(res => {
    ...
    return (
        <div className={styles.additional}>
            {!!isFollowingRequested ? <div className={styles.additionalContent}>
                ...
                {
                    isLoadMoreBtnActive &&
                    <div className={styles.loadMoreBtn} onClick={() => setFetchingFollowing(true)}>Load more</div>
                }
            </div> : null}
        </div>
    )
